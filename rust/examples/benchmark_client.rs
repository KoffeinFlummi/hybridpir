...
fn run_different_element_counts(streams: &mut Vec<TcpStream>, ns: &Vec<usize>, s: usize, b: usize) {
    for n in ns.iter() {
        ...
        if *n >= (1 << 14) {
            ...
            println!("{:?};{:?};{:?}", time_raidpir, time_sealpir, time_hybridpir);
        } else {
            println!("{:?};{:?};", time_raidpir, time_sealpir);
        }
    }
}
...
fn run_benchmarks(mut streams: Vec<TcpStream>) -> Result<(), Error> {
    ...
    //let ns: Vec<usize> = (10..26).step_by(2).map(|x| 1 << x).collect();
    let ns: Vec<usize> = (14..26).step_by(2).map(|x| 1 << x).collect();
    run_different_element_counts(&mut streams, &ns, 8, 1024);
